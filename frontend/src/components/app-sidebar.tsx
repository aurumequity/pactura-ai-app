...
import { LayoutDashboard, FileText, Settings, ClipboardList, LogOut, ShieldCheck, Building2, ChevronDown, Check } from "lucide-react"
...
const NAV_ITEMS = [
  ...
  { label: "Documents",  href: "/documents",  icon: FileText,        roles: null },
  { label: "Audit Logs", href: "/audit-logs", icon: ClipboardList,   roles: ["org_admin", "compliance_officer", "auditor"] },
  ...
  return (
    <aside className="flex h-screen w-64 shrink-0 flex-col bg-sidebar text-sidebar-foreground">
      ...
      <nav className="flex flex-1 flex-col gap-1 px-3 py-4" role="navigation" aria-label="Main navigation">
        {NAV_ITEMS
          .filter((item) =>
            item.roles === null || (org?.role != null && item.roles.includes(org.role))
          )
          .map((item) => {
